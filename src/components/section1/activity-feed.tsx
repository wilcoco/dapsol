...
export function ActivityFeed({ onSelectQASet }: ActivityFeedProps) {
  ...
  const formattedTimes = useMemo(() => {
    ...
    for (const item of feed) {
      ...
      if (mins < 1) result[item.id] = "방금";
        ...
        if (hours < 24) result[item.id] = `${hours}시간 전`;
        else result[item.id] = `${Math.floor(hours / 24)}일 전`;
      }
    }
    return result;
  }, [feed, now]);
  ...
  // Format progressive text: "N명 걸어가는 중" style
  const progressiveMessage = (item: FeedItem) => {
    if (item.action === "invest" && item.amount) {
      return `${item.userName}님이 ${item.amount}👣 걸어가는 중`;
    }
    if (item.action === "hunt" && item.amount) {
      return `${item.userName}님이 반대 중 📉`;
    }
    return item.message;
  };
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground/70 mt-0.5">{formattedTimes[item.id]}</p>
              </div>
            </button>
          );
        })}
